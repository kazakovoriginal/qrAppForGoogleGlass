...
/**
 * This object wraps (охватывает/покрывает) the Camera service object and expects to be the only one
 * talking to it. The
 * implementation encapsulates (изолировать) the steps needed to take preview-sized images,
 * which are used for
...
public final class CameraManager {
     ...
     * where to place the
     * barcode. This target helps with alignment (Этот метод помогает с выравниванием ) as well as forces the user to
     * hold the device
     * far enough away to ensure the image will be in focus.
     *
     * @return The rectangle to draw on screen in window coordinates.
     */
    public synchronized Rect getFramingRect() { //создает рамку, в которую надо поместить qr код
        if (framingRect == null) {
            if (camera == null) {
                return null;
            }
...
//                    topOffset + height);
            framingRect = new Rect(20, 20, 620,
                    340);
